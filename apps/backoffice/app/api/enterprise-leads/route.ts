...
export async function GET(request: Request) {
  try {
    ...
    let query = supabase.from('enterprise_leads').select('*');
    ...
    console.error('Error in GET /api/enterprise-leads:', error);
    return NextResponse.json(
...
export async function PATCH(request: Request) {
  try {
    ...
    if (updates.estimatedLocations !== undefined)
      dbUpdates.estimated_locations = updates.estimatedLocations;
    ...
    if (updates.convertedOrganizationId !== undefined)
      dbUpdates.converted_organization_id = updates.convertedOrganizationId;

...
import { SectionListRenderer } from '../components/site';
...
export default function HomePage() {
  ...
  // Site Configuration (dynamic sections)
  const [siteConfig, setSiteConfig] = useState<SiteConfig>(() => getStaticSiteConfig());
  ...
  useEffect(() => {
    ...
    fetchSiteConfig()
      .then((config) => {
        if (config.sections.length > 0 && !config.error) {
          setSiteConfig(config);
          setUseDynamicSections(true);
        }
      })
      .catch((err) => {
        console.error('[HomePage] Failed to load site config:', err);
  ...
  return (
    <div className="bg-theme-bg-secondary min-h-screen pb-28">
      ...
      <div className="container relative z-10 mx-auto px-4 pt-6">
        ...
        {useDynamicSections ? (
          <SectionListRenderer sections={siteConfig.sections} />
        ) : (
              ...
              >
                <div className="pt-4">
                  ...
                  <p className="text-theme-text-secondary leading-relaxed">
                    ...
                    both body and planet. All our ingredients are fresh, locally sourced when
                    possible, and prepared with love. Come experience the power of plants in Đà
                    Nẵng!
                  </p>
                </div>
              </div>

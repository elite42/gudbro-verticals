...
must_haves:
  truths:
    - 'A guest entering a valid booking code sees their property name, check-in, and check-out dates'
    - 'A guest entering an invalid booking code sees a clear not-found error'
    - 'A guest who verifies with correct last name receives a session token and full stay details'
    - 'A guest who verifies with wrong last name is rejected without revealing whether the code exists'
    - 'All API responses follow a consistent { data, error } shape'
  artifacts:

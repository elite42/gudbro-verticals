...
### 2025-12-07: Multi-Locale System Sprints 3-5 COMPLETED

**Sprint 3 - PWA Dynamic Languages:** ✅
- Language selector in HomeHeader already uses `useMerchantConfig()` for dynamic languages
- MerchantConfigProvider already integrated at app root
- Updated `useTranslation` hook to validate languages against merchant's enabled languages
- Added helper functions: `hasUITranslations()`, `getAvailableUITranslations()`, `validateLanguage()`
- Updated `useDirection` hook to read language directly from `languagePreferencesStore`
- RTL support now works for languages without UI translations (ar, he, fa, ur, ps, dv)

**Files Modified:**
- `apps/coffeeshop/frontend/lib/use-translation.ts` - Added merchant language validation
- `apps/coffeeshop/frontend/lib/hooks/useDirection.ts` - Decoupled from useTranslation for RTL support

**RTL Languages Supported:** Arabic (ar), Hebrew (he), Persian (fa), Urdu (ur), Pashto (ps), Divehi (dv)

**Sprint 4 - Backoffice Translation Editor:** ✅ (Already Implemented)
- `/translations` page - Full translation editor with inline editing for menu_item and category translations
- `/settings/languages` page - Enable/disable languages, set primary language
- CSV export/import with validation
- 111 languages from database with RTL badges

**Files (Already Existed):**
- `apps/backoffice/app/(dashboard)/translations/page.tsx` - Translation editor (37KB)
- `apps/backoffice/app/(dashboard)/settings/languages/page.tsx` - Language settings

**Sprint 5 - Currency System:** ✅ (Already Implemented)
- PWA currency converter with Supabase integration
- 24-hour localStorage cache with fallback rates
- VND compact "k" format (65000 → "65k")
- Dual price display (tourist currency + original)
- Backoffice currency settings page with exchange rates table
- API endpoint for manual rate refresh
- 33+ currencies supported

**Currency System Architecture:**
```
PWA                          Supabase                    Backoffice
┌─────────────────┐          ┌─────────────────┐         ┌─────────────────┐
│currency-converter│◀────────│ exchange_rates  │◀────────│  /api/refresh   │
│                 │          │  (33 currencies)│         │  (POST manual)  │
│ • VND base      │          │ • USD base      │         │ • Currency page │
│ • 24h cache     │          │ • Daily updates │         │ • Refresh btn   │
│ • Fallback rates│          │ • History table │         │ • Stale warning │
└─────────────────┘          └─────────────────┘         └─────────────────┘
```

**Files (Already Existed):**
- `apps/coffeeshop/frontend/lib/currency-converter.ts` - PWA conversion logic
- `apps/backoffice/app/api/exchange-rates/refresh/route.ts` - API endpoint
- `apps/backoffice/app/(dashboard)/settings/currency/page.tsx` - Currency UI
- `shared/database/migrations/009-exchange-rates.sql` - DB schema + helper functions

**Note:** Daily rate updates currently require manual refresh. Automated cron can be added via Vercel if needed.

**All 5 Multi-Locale Sprints: COMPLETED**
...
### Sprint 3: PWA Dynamic Languages (Week 3) ✅ COMPLETED
**Goal:** Languages fetched from merchant config

| Task | Effort | Status |
|------|--------|--------|
| Refactor useTranslation hook | 4h | ✅ Done |
| Dynamic language selector | 3h | ✅ Done (already in HomeHeader) |
| Translation fallback logic | 2h | ✅ Done |
| RTL support (Arabic, Hebrew) | 4h | ✅ Done |
| Fetch translations from database | 3h | Deferred (Sprint 4) |

**Note:** Most infrastructure was already in place. Sprint focused on:
- Validating language against merchant's enabled languages
- Decoupling RTL from UI translations (6 RTL languages supported)

### Sprint 4: Backoffice Translation Editor (Week 4) ✅ COMPLETED
**Goal:** Merchants can manage translations

| Task | Effort | Status |
|------|--------|--------|
| Translation editor UI | 6h | ✅ Done |
| Bulk translation import (CSV) | 3h | ✅ Done |
| Language settings page | 2h | ✅ Done |
| Preview in multiple languages | 3h | Deferred (Sprint 5) |

**Already Implemented:**
- `/translations` page - Full translation editor with inline editing
- `/settings/languages` page - Enable/disable languages, set primary
- CSV export/import with validation
- 111 languages from database with RTL support

### Sprint 5: Currency System Refactor (Week 5) ✅ COMPLETED
**Goal:** Proper merchant→tourist conversion + live rates

| Task | Effort | Status |
|------|--------|--------|
| Refactor currency-converter.ts | 4h | ✅ Done (Already implemented) |
| Exchange rate API integration | 3h | ✅ Done (Supabase + fallback) |
| Backoffice currency settings | 4h | ✅ Done (Manual refresh via UI) |
| VND compact format ("k") | 2h | ✅ Done |
| Multi-currency display (dual) | 3h | ✅ Done |

**Already Implemented:**
- `lib/currency-converter.ts` - Full PWA converter with Supabase integration
- `/api/exchange-rates/refresh/route.ts` - API for manual rate updates
- `/settings/currency/page.tsx` - Backoffice UI with 33+ currencies
- 24-hour localStorage cache with automatic fallback rates
- VND "k" format and dual price display

**Note:** Daily cron job not implemented (manual refresh sufficient for MVP)

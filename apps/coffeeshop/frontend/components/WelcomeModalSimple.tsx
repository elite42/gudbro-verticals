 ...
 * - No swipe-to-dismiss (removed useSwipeToDismiss hook)
 * - No focus trap (simplified)
 * - AuthModal state managed separately
...
export function WelcomeModalSimple({
  ...
  // Auth state - completely independent
  ...
  function handleSignupClick(e: React.MouseEvent) {
    e.stopPropagation();
  ...
  return (
    <>
                ...
                <div className="bg-theme-bg-tertiary border-2 border-theme-border-secondary rounded-xl p-4 shadow-sm">
                  {currentUser ? (
                    ...
                    <>
                      ...
                      {/* Auth Buttons - Simple click handlers */}
      ...
      {/* Auth Modal - Completely separate, shown on top */}
      {showAuthModal && (
        <AuthModal
          isOpen={showAuthModal}

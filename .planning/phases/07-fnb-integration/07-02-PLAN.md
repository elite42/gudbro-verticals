...
2. **CRITICAL: Explicitly extract F&B fields in the PropertyExtended merge block.**
   The existing pattern (around lines 47-52) uses `as unknown as PropertyExtended` casting to extract extended fields. F&B fields MUST be extracted the same way, otherwise they will be undefined at runtime.

   In the `fetchProperty` callback where the `extended` object is constructed, add these two fields alongside the existing `quickActions`, `returnBannerText`, and `returnBannerUrl` extractions:

   ```typescript
   const extended: PropertyExtended = {
     ...data.property,
     quickActions:
       (data.property as unknown as PropertyExtended).quickActions || [],
     returnBannerText:
       (data.property as unknown as PropertyExtended).returnBannerText || null,
     returnBannerUrl:
       (data.property as unknown as PropertyExtended).returnBannerUrl || null,
     hasLinkedFnb:
       (data.property as unknown as PropertyExtended).hasLinkedFnb ?? false,
     linkedFnbSlug:
       (data.property as unknown as PropertyExtended).linkedFnbSlug ?? null,
   };
   ```

   Without this, `propertyExtended?.hasLinkedFnb` will always be `undefined` and the deep-link branch will never activate.

3. Insert RestaurantSection between QuickActions and ServicesCarousel (around line 100):
...
4. Add NEXT_PUBLIC_COFFEESHOP_BASE_URL to .env.local (or .env.example if it exists) with value `http://localhost:3004` for local development. Check if .env.local or .env.example exists first -- if neither exists, create .env.example with this variable documented.

5. Run full build to verify everything compiles:
...
Read page.tsx and confirm:

- RestaurantSection is imported and placed between QuickActions and ServicesCarousel
- PropertyExtended merge block explicitly extracts hasLinkedFnb and linkedFnbSlug (grep for "hasLinkedFnb:" in the extended object construction)
- NEXT_PUBLIC_COFFEESHOP_BASE_URL is set in .env.local or documented in .env.example
  </verify>
  <done>Dashboard page includes RestaurantSection between QuickActions and ServicesCarousel. F&B fields explicitly extracted in PropertyExtended merge. NEXT_PUBLIC_COFFEESHOP_BASE_URL documented. Accommodations frontend builds successfully with zero errors.</done>
  </task>
...
- PropertyExtended merge block explicitly extracts hasLinkedFnb and linkedFnbSlug fields (not relying on spread)

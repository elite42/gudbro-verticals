      ...
      <div className="flex items-center justify-between px-4 py-3">
        ...
        <div className="flex items-center gap-3">
          {showBack ? (
            <button
              onClick={onBack}
              className="touch-target -ml-2 rounded-lg p-2 transition-colors hover:bg-gray-100"
              aria-label="Go back"
            >
              <svg
                className="h-6 w-6"
            ...
            <Link href="/" className="flex items-center gap-2">
              {/* GUDBRO Tours Logo */}
              <div className="from-primary to-accent flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br text-sm font-bold text-white">
                GT
              </div>
              {operatorName && (
                <div className="flex items-center gap-1.5">
                  <span className="font-display text-foreground max-w-[120px] truncate font-semibold">
                    {operatorName}
                  </span>
                  {operatorVerified && (
                    <svg
          ...
          <div className="relative">
            ...
            {showLangMenu && (
              <>
                ...
                <div
                  className="border-border animate-scale-in absolute right-0 top-full z-20 mt-1 w-48 origin-top-right rounded-xl border bg-white py-1 shadow-lg"
                  role="listbox"
                >
                  {LANGUAGES.map((lang) => (
                    <button
          ...
          <div className="relative">
            ...
            {showCurrMenu && (
              <>
                ...
                <div
                  className="border-border animate-scale-in absolute right-0 top-full z-20 mt-1 w-56 origin-top-right rounded-xl border bg-white py-1 shadow-lg"
                  role="listbox"
                >
                  {CURRENCIES.map((curr) => (
                    <button
                      ...
                      className={cn(
                        'flex w-full items-center gap-3 px-4 py-2.5 text-left transition-colors',
                        currency === curr.code ? 'bg-primary/10 text-primary' : 'hover:bg-gray-50'
                      )}
                      role="option"
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div className="text-foreground-muted text-xs">{curr.name}</div>
                      </div>
                      {currency === curr.code && (
                        <svg className="ml-auto h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
                          <path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z" />

#!/usr/bin/env python3
"""
Generates SQL INSERT statements for product_ingredients from Thai data files.
"""
...
def main():
    ...
    # Write output file
    output_file = os.path.join(scripts_dir, '04-thai-product-ingredients.sql')
    with open(output_file, 'w') as f:
        f.write("-- ============================================\n")
        f.write("-- THAI PRODUCT_INGREDIENTS LINKS\n")
        f.write("-- ============================================\n")
        ...
        f.write("COMMIT;\n\n")
        f.write("-- Verify\n")
        f.write("SELECT COUNT(*) as thai_links FROM product_ingredients WHERE product_type = 'thai';\n")
        f.write("SELECT COUNT(DISTINCT product_id) as thai_dishes_with_ingredients FROM product_ingredients WHERE product_type = 'thai';\n")

...
must_haves:
  ...
  key_links:
    ...
    - from: 'apps/accommodations/frontend/hooks/useBookingForm.ts'
      to: '/api/checkout'
      via: 'fetch POST after card booking to create Stripe session'
...
- After successful submission, handle card payment redirect:
  1. If response has `stripeCheckoutUrl` that is non-null, redirect via `window.location.href = stripeCheckoutUrl`
  2. If response has `paymentMethod === 'card'` and `stripeCheckoutUrl` is null, call `/api/checkout` POST with `{ bookingId: response.bookingId }` to create a Stripe Checkout session, then redirect to the returned URL
  3. For non-card methods, proceed to confirmation page as normal
- Return: `selectedPaymentMethod`, `setSelectedPaymentMethod` in the hook return
  </action>
  <verify>

1. Run `cd /Users/gianfrancodagostino/Desktop/gudbro-verticals/apps/accommodations/frontend && npx tsc --noEmit --pretty 2>&1 | head -40` to check TypeScript compilation.
2. Verify BookingForm imports PaymentMethodSelector: `grep 'PaymentMethodSelector' apps/accommodations/frontend/components/booking/BookingForm.tsx` should match import and render.
3. Verify useBookingForm exposes selectedPaymentMethod: `grep 'selectedPaymentMethod' apps/accommodations/frontend/hooks/useBookingForm.ts` should match state declaration and return.
   </verify>
   <done>PaymentMethodSelector renders filtered payment options. BankTransferInstructions shows bank details. CryptoPaymentOptions shows wallet addresses with deep-links. BookingForm imports and renders PaymentMethodSelector. useBookingForm tracks selectedPaymentMethod state and passes it to API. Card payments trigger Stripe redirect.</done>
   </task>

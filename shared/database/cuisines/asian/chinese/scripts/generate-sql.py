#!/usr/bin/env python3
"""
Generates SQL INSERT statements for Chinese dishes from TypeScript data files.
"""
...
def parse_ts_object(ts_obj):
    ...
    string_patterns = {
        ...
        'chinese_name': r"chinese_name:\s*['\"]([^'\"]+)['\"]",
        'chinese_script': r"chinese_script:\s*['\"]([^'\"]+)['\"]",
        'category': r"category:\s*['\"]([^'\"]+)['\"]",
        'status': r"status:\s*['\"]([^'\"]+)['\"]",
        'region': r"region:\s*['\"]([^'\"]+)['\"]",
        'cuisine_style': r"cuisine_style:\s*['\"]([^'\"]+)['\"]",
    ...
    bool_patterns = {
        ...
        'is_dim_sum': r"is_dim_sum:\s*(true|false)",
...
def generate_insert(item):
    """Generate SQL INSERT statement for a Chinese dish."""
    required = ['id', 'slug', 'name', 'description', 'category', 'status',
                'region', 'cuisine_style', 'protein_type', 'cooking_method']
  ...
  {f"'{escape_sql(item['chinese_name'])}'" if item.get('chinese_name') else 'NULL'},
  {f"'{escape_sql(item['chinese_script'])}'" if item.get('chinese_script') else 'NULL'},
  '{escape_sql(item['category'])}',
  '{escape_sql(item['status'])}',
  '{escape_sql(item['region'])}',
  '{escape_sql(item['cuisine_style'])}',
  '{escape_sql(item['protein_type'])}',
  '{escape_sql(item['cooking_method'])}',
  {item.get('is_street_food', 'false')},
  {item.get('is_dim_sum', 'false')},
...
def main():
    ...
    # Write output file
    output_file = os.path.join(scripts_dir, '03-chinese-data.sql')
    with open(output_file, 'w') as f:
        f.write("-- ============================================\n")
        f.write("-- CHINESE DATA IMPORT\n")
        f.write("-- ============================================\n")
        f.write(f"-- Total: {len(all_inserts)} Chinese dishes\n")
        ...
        f.write("COMMIT;\n\n")
        f.write("-- Verify\n")
        f.write("SELECT category, COUNT(*) as count FROM chinese GROUP BY category ORDER BY count DESC;\n")
        f.write("SELECT COUNT(*) as total_chinese_dishes FROM chinese;\n")

    print(f"\nGenerated {output_file}")
    print(f"Total Chinese dishes: {len(all_inserts)}")

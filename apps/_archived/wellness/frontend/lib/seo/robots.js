...
export function generateRobotsTxt(config) {
  const { sitemapUrl, allowAll = true, disallowPaths = [], crawlDelay, specificBots = {} } = config;
...
export function generateAdvancedRobots(baseUrl) {
  return generateRobotsTxt({
    sitemapUrl: `${baseUrl}/sitemap.xml`,
    allowAll: true,
    disallowPaths: ['/api/', '/admin/', '/_next/', '/checkout/', '/cart/'],
    specificBots: {
...
export function createRobotsRoute(baseUrl, isProduction = true) {
  return function GET() {
    const robots = isProduction ? generateProductionRobots(baseUrl) : generateDevelopmentRobots();
...
export function validateRobotsTxt(content) {
  ...
  if (sitemapMatches && sitemapMatches.length > 5) {
    warnings.push(
      `Too many sitemap entries (${sitemapMatches.length}). Consider using sitemap index.`
    );
  }

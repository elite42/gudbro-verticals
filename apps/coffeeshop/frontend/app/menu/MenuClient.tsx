...
function getLocalizedText(
  multi: MultilingualText | undefined,
  fallback: string,
  lang: string
): string {
...
export default function MenuClient({ initialMenuItems }: MenuClientProps) {
      ...
      icon: (
        <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path
            ...
            strokeLinejoin="round"
            strokeWidth={2}
            d="M21 15a2 2 0 01-2 2H5a2 2 0 01-2-2m18 0a9 9 0 10-18 0"
          />
        </svg>
      ),
    },
      ...
      icon: (
        <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path
            ...
            strokeLinejoin="round"
            strokeWidth={2}
            d="M19 2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h4l3 3 3-3h4c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm-7 3.5c1.93 0 3.5 1.57 3.5 3.5s-1.57 3.5-3.5 3.5S8.5 10.93 8.5 9 10.07 5.5 12 5.5z"
          />
        </svg>
      ),
    },
      ...
      icon: (
        <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path
            ...
            strokeLinejoin="round"
            strokeWidth={2}
            d="M5 3h14l-2 8H7L5 3zm0 0c0 1.5 1 2.5 2.5 2.5S10 4.5 10 3M7 11l-1 10h12l-1-10"
          />
        </svg>
      ),
    },
  ...
  return (
    <div className="bg-theme-bg-secondary min-h-screen pb-28 lg:pb-8">
      {/* Header */}
      <MenuHeader selectionsCount={selectionsCount} onSelectionsClick={openSelectionsSidebar} />

      {/* Table Context Bar (when QR scanned) */}
      {isClient && tableContext.table_number && (
        <div className="from-theme-brand-primary to-theme-brand-primary bg-gradient-to-r px-4 py-3 text-white shadow-md">
          <div className="container mx-auto flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                {tableContext.customer_name && (
                  <div className="text-sm opacity-90">{tableContext.customer_name}</div>
                )}
              </div>
            </div>
            ...
            <div className="rounded-full bg-white/20 px-3 py-1 text-sm font-medium">
              {tableContext.consumption_type === 'dine-in'
                ? `🏠 ${t.menu.tableContext.dineIn}`
                : `📦 ${t.menu.tableContext.takeaway}`}
            </div>
          </div>
        </div>
      )}
      ...
      {isClient &&
        (() => {
          ...
          const grouped = itemsToDisplay.reduce(
            (acc, item) => {

...
export function PaymentForm({
  ...
  return (
    <div className="space-y-6">
      ...
      {method === 'cash' && (
        <CashPaymentInfo amount={amount} onConfirm={() => handleSubmit({})} />
      )}
      ...
      {/* Error display */}
      {error && (
        <div className="p-3 bg-error/10 border border-error/30 rounded-xl text-error text-sm">
          {error}
        </div>
      )}
    ...
    <div className="space-y-4">
      <div className="p-4 bg-success/10 border border-success/30 rounded-xl">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h4 className="font-semibold text-foreground">Pay on Pickup</h4>
            <p className="text-sm text-foreground-muted mt-1">
              Pay your guide when they pick you up. Cash (VND or USD) and
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3 p-3 bg-blue-50 rounded-xl">
        ...
        <div>
          ...
          <p className="text-xs text-foreground-muted">Vietnamese banks & cards</p>
        </div>
      </div>

      <div>
        <p className="text-sm font-medium text-foreground-muted mb-2">Select your bank</p>
        <div className="grid grid-cols-3 gap-2">
          {banks.map(bank => (
            ...
            >
              ...
              <p className="text-xs font-medium mt-1">{bank.code}</p>
            </button>
          ))}
        </div>
      </div>
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3 p-3 bg-pink-50 rounded-xl">
        ...
        <div>
          ...
          <p className="text-xs text-foreground-muted">Vietnam's #1 e-wallet</p>
        </div>
      </div>
      ...
      <p className="text-xs text-center text-foreground-muted">
        Confirm payment in your MoMo app
      </p>
    </div>
  )
}
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3 p-3 bg-indigo-50 rounded-xl">
        <div className="flex gap-1">
          <div className="w-8 h-5 bg-blue-700 rounded flex items-center justify-center text-white text-xs font-bold">
            VISA
          </div>
          <div className="w-8 h-5 bg-red-500 rounded-full flex items-center justify-center">
            <div className="w-3 h-3 bg-yellow-400 rounded-full opacity-80" />
          </div>
        </div>
        <div>
          <p className="font-semibold text-foreground">Credit/Debit Card</p>
          <p className="text-xs text-foreground-muted">Powered by Stripe</p>
        </div>
      </div>
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3 p-3 bg-orange-50 rounded-xl">
        ...
        <div>
          ...
          <p className="text-xs text-foreground-muted">Fast & borderless</p>
        </div>
      </div>

      {/* Crypto selection */}
      <div>
        <p className="text-sm font-medium text-foreground-muted mb-2">Select cryptocurrency</p>
          ...
          {cryptos.map(crypto => (
            ...
            >
              <div className="flex items-center gap-2">
                <div className={cn('w-8 h-8 rounded-full flex items-center justify-center text-white font-bold', crypto.color)}>
                  {crypto.icon}
                </div>
                <div className="text-left">
                  <p className="font-medium text-sm">{crypto.symbol}</p>
                  <p className="text-xs text-foreground-muted">{crypto.name}</p>
                </div>
              </div>
              {selectedCrypto === crypto.id && (
      ...
      {selectedCrypto && (
        <div className="p-4 bg-gray-50 rounded-xl">
          <p className="text-sm font-medium mb-2">Send to this address:</p>
          <div className="flex items-center gap-2">
            <code className="flex-1 text-xs bg-white p-2 rounded border border-border font-mono break-all">

...
| Menu/Category  | Prodotti per categoria        | Filtro            |
| Menu/Popular   | Top 12 prodotti popolari      | Quick pick        |
| Cart           | Carrello ordine               | Prima di ordinare |
| Orders         | Storico + tracking real-time  | Post-ordine       |
| Account        | Profilo, preferenze           | Setup/gestione    |
| Offers         | Promozioni attive             | Decisione         |
| Events         | Eventi del locale             | Scoperta          |
| Team           | Staff con reviews             | Curiosità/tip     |
| About          | Storia e valori del locale    | Scoperta          |
| Contact        | Form contatti + Google Maps   | Comunicazione     |
| Reset Password | Recupero password             | Auth              |

**Real-time Features (v2):**
- **Order Tracking**: Progress bar animata con 5 step (pending → confirmed → preparing → ready → delivered)
- **ETA Display**: Tempo stimato con confidence level e auto-refresh ogni 30s
- **Push Notifications**: Prompt per abilitare notifiche quando ordine attivo
- **Sound Notifications**: Suoni al cambio stato ordine (ready, update, success)
...
- v1.8 (2026-01-25): PWA v2 Feature Parity
  - Sezione 6.3 espansa con nuove pagine PWA (Category, Popular, About, Contact, etc.)
  - Aggiunte Real-time Features: Order Tracking, ETA Display, Push/Sound Notifications
- v1.7 (2026-01-23): Minor updates

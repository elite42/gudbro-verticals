...
const MOCK_FLEET = [
  // SCOOTERS (110-125cc)
  {
    ...
    isFeatured: true,
    pricing: {
      'loc-001': { dailyRate: 120000, weeklyRate: 700000, monthlyRate: 2400000, longTermRate: 6000000, depositAmount: 500000 },
      'loc-002': { dailyRate: 130000, weeklyRate: 750000, monthlyRate: 2500000, longTermRate: 6200000, depositAmount: 500000 }
    },
    inventory: {
      'loc-001': { quantityTotal: 5, quantityAvailable: 3, quantityReserved: 2, quantityMaintenance: 0 },
      'loc-002': { quantityTotal: 3, quantityAvailable: 2, quantityReserved: 1, quantityMaintenance: 0 }
    }
  },
  {
    id: 'item-002',
    ...
    isFeatured: false,
    pricing: {
      'loc-001': { dailyRate: 130000, weeklyRate: 800000, monthlyRate: 2600000, longTermRate: 6500000, depositAmount: 600000 }
    },
    salePrice: { price: 25000000, isNegotiable: true, minPrice: 23000000 },
    inventory: {
      'loc-001': { quantityTotal: 4, quantityAvailable: 4, quantityReserved: 0, quantityMaintenance: 0 }
    }
  },
  // SPORT BIKES (150cc+)
  {
    ...
    isFeatured: true,
    pricing: {
      'loc-001': { dailyRate: 200000, weeklyRate: 1200000, monthlyRate: 4000000, longTermRate: 10000000, depositAmount: 1000000 },
      'loc-002': { dailyRate: 220000, weeklyRate: 1300000, monthlyRate: 4200000, longTermRate: 10500000, depositAmount: 1000000 }
    },
    inventory: {
      'loc-001': { quantityTotal: 3, quantityAvailable: 1, quantityReserved: 2, quantityMaintenance: 0 },
      'loc-002': { quantityTotal: 2, quantityAvailable: 2, quantityReserved: 0, quantityMaintenance: 0 }
    }
  },
  // ELECTRIC
  {
    ...
    year: 2024,
    condition: 'excellent',
    specs: { battery: '60V', range: '80km', chargingTime: '6h', transmission: 'automatic', seats: 2 },
    ...
    isFeatured: true,
    pricing: {
      'loc-001': { dailyRate: 150000, weeklyRate: 900000, monthlyRate: 3000000, longTermRate: 7500000, depositAmount: 800000 },
      'loc-002': { dailyRate: 160000, weeklyRate: 950000, monthlyRate: 3200000, longTermRate: 8000000, depositAmount: 800000 }
    },
    inventory: {
      'loc-001': { quantityTotal: 4, quantityAvailable: 3, quantityReserved: 1, quantityMaintenance: 0 },
      'loc-002': { quantityTotal: 2, quantityAvailable: 1, quantityReserved: 0, quantityMaintenance: 1 }
    }
  },
  // BICYCLES
  {
    ...
    isFeatured: false,
    pricing: {
      'loc-001': { dailyRate: 80000, weeklyRate: 450000, monthlyRate: 1500000, longTermRate: 3500000, depositAmount: 300000 },
      'loc-002': { dailyRate: 90000, weeklyRate: 500000, monthlyRate: 1600000, longTermRate: 3800000, depositAmount: 300000 }
    },
    inventory: {
      'loc-001': { quantityTotal: 6, quantityAvailable: 5, quantityReserved: 1, quantityMaintenance: 0 },
      'loc-002': { quantityTotal: 4, quantityAvailable: 4, quantityReserved: 0, quantityMaintenance: 0 }
    }
  },
  {
    id: 'item-006',
    ...
    isFeatured: false,
    pricing: {},
    salePrice: { price: 8000000, isNegotiable: true, minPrice: 7500000, conditionNotes: 'Used for 1 year, good condition, no major scratches' },
    inventory: {
      'loc-001': { quantityTotal: 1, quantityAvailable: 1, quantityReserved: 0, quantityMaintenance: 0 }
    }
  }
];
...
function calculateDurationPrice(dailyRate, weeklyRate, monthlyRate, longTermRate, durationDays) {
  if (durationDays >= 90 && longTermRate) {
    return { totalPrice: Math.ceil((durationDays / 90) * longTermRate), pricePerDay: Math.ceil(longTermRate / 90), tier: 'long-term' };
  } else if (durationDays >= 30 && monthlyRate) {
    return { totalPrice: Math.ceil((durationDays / 30) * monthlyRate), pricePerDay: Math.ceil(monthlyRate / 30), tier: 'monthly' };
  } else if (durationDays >= 7 && weeklyRate) {
    return { totalPrice: Math.ceil((durationDays / 7) * weeklyRate), pricePerDay: Math.ceil(weeklyRate / 7), tier: 'weekly' };
  } else {
    ...
    try {
      ...
      if (type) {
        fleet = fleet.filter(item =>
          item.itemType === type || item.itemType === 'both'
        );
      }
      ...
      if (location && location !== 'all') {
        fleet = fleet.filter(item =>
          item.inventory[location] && item.inventory[location].quantityAvailable > 0
        );
      }
        ...
        if (location === 'all') {
          ...
          if (allPricing.length > 0) {
            pricingData = {
              ...
              weeklyRate: allPricing[0].weeklyRate ? Math.min(...allPricing.filter(p => p.weeklyRate).map(p => p.weeklyRate)) : null,
              monthlyRate: allPricing[0].monthlyRate ? Math.min(...allPricing.filter(p => p.monthlyRate).map(p => p.monthlyRate)) : null,
              longTermRate: allPricing[0].longTermRate ? Math.min(...allPricing.filter(p => p.longTermRate).map(p => p.longTermRate)) : null,
              depositAmount: Math.min(...allPricing.filter(p => p.depositAmount).map(p => p.depositAmount))
            };
          }
...
${message}`;
      ...
      const primaryUrl = contactUrls[preferredChannel] || contactUrls.zalo || contactUrls.whatsapp || contactUrls.email;

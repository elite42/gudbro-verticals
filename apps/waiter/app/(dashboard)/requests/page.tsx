...
import { useRequestsStore, selectPendingCount, type CustomerRequest } from '@/lib/stores/requests-store';
import { acknowledgeRequest, completeRequest, fetchRequests } from '@/lib/services/requests-service';
...
export default function RequestsPage() {
  ...
  const filteredRequests = requests.filter((r) => {
    ...
    if (searchQuery) {
      ...
      if (!r.tableNumber.toLowerCase().includes(query) &&
    ...
    if (filterStatus === 'acknowledged' &&
        r.status !== 'acknowledged' && r.status !== 'in_progress') return false;
          ...
          {[1, 2, 3].map((i) => (
            <div key={i} className="card p-4 animate-pulse">
              <div className="flex gap-3">
                <div className="w-12 h-12 bg-theme-bg-tertiary rounded-xl" />

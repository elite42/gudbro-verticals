...
export function TourDetail({ tour, operator, currency, onBookNow }: TourDetailProps) {
  ...
  return (
    <div className="pb-24">
      ...
      <section className="relative">
        {/* Main Image */}
        <div className="relative aspect-[4/3] overflow-hidden bg-gray-100 md:aspect-[16/9]">
          <Image
      ...
      <div className="mx-auto max-w-3xl px-4 md:px-6">
        ...
        <header className="border-border border-b py-6">
          ...
          <h1 className="font-display text-foreground mb-3 text-2xl font-bold md:text-3xl">
            {tour.name}
          </h1>
        ...
        {/* Quick Info Bar */}
        <section className="border-border flex flex-wrap gap-6 border-b py-4">
        ...
        {/* Description */}
        <section className="border-border border-b py-6">
          <h2 className="font-display mb-4 text-xl font-semibold">About This Tour</h2>
        ...
        {/* Included / Excluded */}
        <section className="border-border grid gap-6 border-b py-6 md:grid-cols-2">
          {/* Included */}
          <div>
            <h3 className="font-display text-secondary mb-4 flex items-center gap-2 text-lg font-semibold">
            ...
            <ul className="space-y-3">
              {tour.included.map((item, index) => (
                <li key={index} className="text-foreground-muted flex items-start gap-3">
          ...
          {/* Excluded */}
          <div>
            <h3 className="font-display text-error mb-4 flex items-center gap-2 text-lg font-semibold">
              <svg className="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
        ...
        {tour.departure_times && tour.departure_times.length > 0 && (
          <section className="border-border border-b py-6">
            <h2 className="font-display mb-4 text-xl font-semibold">Departure Times</h2>
            <div className="flex flex-wrap gap-3">
              {tour.departure_times.map((time, index) => (
                <div
                  key={index}
                  className="bg-primary/10 text-primary rounded-xl px-4 py-2 font-medium"
                >
                  {time}
                </div>
              ))}
        ...
        {tour.pickup_locations && tour.pickup_locations.length > 0 && (
          <section className="border-border border-b py-6">
            <h2 className="font-display mb-4 text-xl font-semibold">Pickup Locations</h2>
        ...
        {/* Operator Info */}
        <section className="border-border border-b py-6">
          <h2 className="font-display mb-4 text-xl font-semibold">Your Operator</h2>
          <div className="bg-background-elevated border-border flex items-center gap-4 rounded-2xl border p-4">
              ...
              {[
                {
                  ...
                  name: 'Min-jun',
                  rating: 5,
                  text: 'Amazing experience! The views were incredible.',
                },
                {
                  flag: '🇦🇺',
                  name: 'Sarah',
                  rating: 5,
                  text: 'Minh was very friendly and knowledgeable.',
                },
                {
                  flag: '🇯🇵',
                  name: 'Yuki',
                  rating: 4,
                  text: 'Beautiful views, great value for money.',
                },
              ].map((review, index) => (
                <div
                  key={index}
                  className="bg-background-elevated border-border rounded-xl border p-4"
                >
      ...
      {/* ─────────────────────────────────────────────────────────────────
          STICKY BOOK NOW BAR
         ───────────────────────────────────────────────────────────────── */}
      <div className="border-border safe-bottom fixed inset-x-0 bottom-0 border-t bg-white p-4 shadow-lg">

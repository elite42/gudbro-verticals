...
export async function GET() {
  try {
    ...
    const pointValue = configMap['point_value_eur'] as { value: number; currency: string } || { value: 0.01, currency: 'EUR' };
    const expiryPolicy = configMap['expiry_policy'] as { months_to_expire: number } || { months_to_expire: 24 };

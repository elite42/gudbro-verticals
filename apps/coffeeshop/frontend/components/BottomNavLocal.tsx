  ...
  return (
    <React.Fragment>
      <nav
        ...
        className="bg-theme-bg-elevated border-theme-bg-tertiary fixed bottom-0 left-0 right-0 z-[9999] w-full border-t-2 shadow-lg transition-transform duration-300 ease-in-out lg:hidden"
        style={{
          transform: isVisible && !isModalOpen ? 'translateY(0)' : 'translateY(100%)',
        }}
      >
        <div className="mx-auto flex h-14 max-w-screen-xl items-center justify-around px-4">
          {navItems.map((item, index) => {
            ...
            if (isCenter) {
              return (
                ...
                >
                  <div className="text-theme-text-secondary hover:bg-theme-bg-tertiary flex h-12 w-12 items-center justify-center rounded-full transition-all duration-200">
            ...
            if (!isLink && 'action' in item) {
              return (
                <button
                  ...
                  aria-label={
                    badge && badge > 0
                      ? `View order list, ${badge} item${badge !== 1 ? 's' : ''} saved`
                      : 'View order list'
                  }
                >
                  <div className="text-theme-text-secondary hover:bg-theme-bg-tertiary relative flex h-12 w-12 items-center justify-center rounded-full transition-all duration-200">
                    <span aria-hidden="true">{item.icon}</span>
                    {typeof badge === 'number' && badge > 0 && (
                      <div
                        className="bg-theme-brand-primary absolute -right-0.5 -top-0.5 flex h-[18px] min-w-[18px] items-center justify-center rounded-full px-1 text-xs font-bold text-white"
                        aria-live="polite"
                        aria-atomic="true"
                      >
                        {badge}

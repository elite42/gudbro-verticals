...
const ALL_PRODUCTS = [
  {
    ...
    nameBrand: 'Panadol',
    nameVi: 'Thuốc giảm đau hạ sốt',
    generic: 'Acetaminophen / Paracetamol',
    category: 'Pain Relief',
    categoryKey: 'pain_relief',
    type: 'otc' as const,
    price: 25000,
  ...
  },
  {
    ...
    nameBrand: 'Advil',
    nameVi: 'Thuốc kháng viêm giảm đau',
    generic: 'Ibuprofen',
    category: 'Pain Relief',
    categoryKey: 'pain_relief',
    type: 'otc' as const,
  ...
  },
  {
    ...
    warnings: [
      ...
      'See a doctor if diarrhea persists over 48 hours',
      'Stay hydrated — use ORS',
    ],
  ...
  },
  {
    ...
    warnings: [
      ...
      'Allergic reactions possible — tell pharmacist about any penicillin allergy',
    ],
    ...
    return (
      ...
      >
        <div className="text-center">
          ...
          <h2 className="font-display mb-2 text-lg font-bold" style={{ color: 'var(--charcoal)' }}>
            Product Not Found
          </h2>
          <p className="mb-4 text-sm" style={{ color: 'var(--charcoal-muted)' }}>
  ...
  return (
    <div className="min-h-screen pb-24" style={{ background: 'var(--cloud)' }}>
      ...
      <main className="mx-auto max-w-lg px-4">
        {/* Product Image */}
        <div
          className={`relative mt-4 h-52 overflow-hidden rounded-2xl ${mounted ? 'animate-fade-in-up' : 'opacity-0'}`}
          style={{
        ...
        >
          ...
          {/* Type badge */}
          <span
            className="absolute left-3 top-3 flex items-center gap-1.5 rounded-lg px-3 py-1 text-[11px] font-bold text-white"
        ...
        <section className={`mt-5 ${mounted ? 'animate-fade-in-up delay-100' : 'opacity-0'}`}>
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="mt-0.5 text-sm" style={{ color: 'var(--charcoal-muted)' }}>
                {product.nameVi}
              </p>
            </div>
        ...
        <section className={`mt-5 ${mounted ? 'animate-fade-in-up delay-200' : 'opacity-0'}`}>
          <h3 className="font-display mb-2 text-sm font-bold" style={{ color: 'var(--charcoal)' }}>
            Used For
          </h3>
          ...
          >
            {product.warnings.map((warning, i) => (
              <div key={i} className="flex items-start gap-2">
                <span className="mt-0.5 text-xs" style={{ color: 'var(--amber-dark)' }}>
                  •
                </span>
                <p className="text-xs leading-relaxed" style={{ color: 'var(--charcoal)' }}>
                  {warning}
        ...
        <section className={`mt-5 ${mounted ? 'animate-fade-in-up delay-500' : 'opacity-0'}`}>
          ...
          <div className="shadow-soft rounded-xl bg-white p-4">
            {product.sideEffects.map((effect, i) => (
              <div key={i} className="mb-1 flex items-start gap-2 last:mb-0">
                <span className="mt-0.5 text-xs" style={{ color: 'var(--charcoal-muted)' }}>
                  •
                </span>
        ...
        <section className={`mb-4 mt-6 ${mounted ? 'animate-fade-in-up delay-600' : 'opacity-0'}`}>
          <div className="flex gap-2.5">
            <a
              ...
              className="flex flex-1 items-center justify-center gap-2 rounded-xl py-3.5 text-sm font-semibold text-white transition-transform hover:scale-[1.02]"
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-2 rounded-xl px-5 py-3.5 text-sm font-semibold text-white"
          ...
          >
            <p className="text-[10px] leading-relaxed" style={{ color: 'var(--charcoal-muted)' }}>
              ℹ️ This is for informational purposes only. Always consult a licensed pharmacist
              before purchasing medication. GUDBRO does not sell or dispense medication.
            </p>
          </div>
        </section>
      </main>

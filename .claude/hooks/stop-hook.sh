...
# Installed from: anthropics/claude-code/plugins/ralph-wiggum
...
if [[ ! "$ITERATION" =~ ^[0-9]+$ ]]; then
  echo "Ralph loop: State file corrupted - iteration invalid" >&2
  rm "$RALPH_STATE_FILE"
  exit 0
fi

if [[ ! "$MAX_ITERATIONS" =~ ^[0-9]+$ ]]; then
  echo "Ralph loop: State file corrupted - max_iterations invalid" >&2
  rm "$RALPH_STATE_FILE"
  exit 0
fi
...
if [[ $? -ne 0 ]]; then
  echo "Ralph loop: Failed to parse JSON" >&2
  rm "$RALPH_STATE_FILE"
  exit 0
fi

if [[ -z "$LAST_OUTPUT" ]]; then
  echo "Ralph loop: No text content" >&2
  rm "$RALPH_STATE_FILE"
  exit 0
fi
...
if [[ -z "$PROMPT_TEXT" ]]; then
  echo "Ralph loop: No prompt text found" >&2
  rm "$RALPH_STATE_FILE"
  exit 0
fi

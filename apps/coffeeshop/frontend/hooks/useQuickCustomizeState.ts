...
export function useQuickCustomizeState({ dish, isOpen }: UseQuickCustomizeStateProps) {
  ...
  const hasCustomizations = !!(dish as any).customizations && Array.isArray((dish as any).customizations);
  const customizations: ProductCustomization[] = hasCustomizations ? (dish as any).customizations : [];
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (hasCustomizations) {
        ...
        const consumptionCustomization = customizations.find(c => c.id === 'espresso-consumption');
        if (consumptionCustomization && tableContext.table_number) {
          const consumptionOptionId = tableContext.consumption_type === 'dine-in' ? 'al-tavolo' : 'da-portare-via';
  ...
  }, [dish.id, isOpen, hasCustomizations, tableContext.consumption_type, tableContext.table_number]);
  ...
  const hasAnyCustomization = () => {
    ...
    if (hasCustomizations && Object.keys(customizationState).length > 0) {
      ...
      for (const customizationId in customizationState) {
        ...
        if (customization.type === 'checkbox' && Array.isArray(selectedValue) && selectedValue.length > 0) {
          return true;
        }
      }
    }
  ...
  const convertCustomizationsToExtras = (): Extra[] => {
    ...
    customizations.forEach((customization) => {
      ...
      if (customization.type === 'radio' && selectedValue) {
        const selectedOption = customization.options.find(opt => opt.id === selectedValue);
        if (selectedOption) {
          const optionName = typeof selectedOption.name === 'string' ? selectedOption.name : selectedOption.name.it || selectedOption.name.en || '';
          const optionNameLower = optionName.toLowerCase();

          const isNoneOption = optionNameLower.includes('nessuno') || optionNameLower.includes('none');
          const isDefaultWithNoPrice = selectedOption.is_default && selectedOption.price_modifier === 0;
  ...
  return {
    ...
    savePreference: (extras: Extra[]) => dishPreferencesStore.set(dish.id, dish.name, extras, quantity)
  };
}

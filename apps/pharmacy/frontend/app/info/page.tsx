...
const PHRASES = [
  ...
  { english: "I'm allergic to...", vietnamese: 'Tôi bị dị ứng với...', pronunciation: 'Toy bee zee uhng voy' },
];

const EMERGENCY_NUMBERS = [
  { icon: '🏥', label: 'Emergency', number: '115', href: 'tel:115' },
  { icon: '🚔', label: 'Police', number: '113', href: 'tel:113' },
  { icon: '🏥', label: 'Da Nang General Hospital', number: '+84 236 3822 344', href: 'tel:+842363822344' },
  { icon: '🏥', label: 'Family Medical Practice (English)', number: '+84 28 3822 7848', href: 'tel:+842838227848' },
];
    ...
    >
      ...
      <main className="max-w-lg mx-auto px-4">
        ...
        <section className={`mt-5 ${mounted ? 'animate-fade-in-up' : 'opacity-0'}`}>
          <div
            className="rounded-2xl p-5 bg-white shadow-soft"
        ...
        <section className={`mt-4 ${mounted ? 'animate-fade-in-up delay-100' : 'opacity-0'}`}>
          ...
          >
            ...
            <p
              className="text-[11px] italic leading-relaxed mb-3"
            ...
            >
              <p
                ...
                style={{ color: 'var(--charcoal-light)' }}
              >
                <span className="font-bold" style={{ color: 'var(--amber)' }}>86.8%</span> of Vietnamese
                pharmacies sell antibiotics without prescription, but this is{' '}
        ...
        <section className={`mt-4 ${mounted ? 'animate-fade-in-up delay-200' : 'opacity-0'}`}>
          ...
          >
            ...
            >
              ...
              <span className="font-bold" style={{ color: 'var(--red)' }}>legal issues</span>.
            </p>
          </div>
        </section>
        ...
        <section className={`mt-6 ${mounted ? 'animate-fade-in-up delay-300' : 'opacity-0'}`}>
          ...
          <div className="bg-white rounded-2xl shadow-soft overflow-hidden">
            ...
            >
              ...
              <p
                className="text-[11px] font-bold uppercase tracking-wider text-right min-w-[90px]"
                style={{ color: 'var(--green-dark)' }}
              >
                VND
              </p>
              <p
                className="text-[11px] font-bold uppercase tracking-wider text-right min-w-[80px]"
                style={{ color: 'var(--green-dark)' }}
              >
                USD
              </p>
            ...
            {PRICE_DATA.map((row, i) => (
              <div
                key={row.medicine}
                className="grid grid-cols-[1fr_auto_auto] gap-2 px-4 py-3 border-t"
                style={{
              ...
              >
                ...
                <p
                  className="text-[12px] text-right min-w-[90px]"
                  style={{ color: 'var(--charcoal-light)' }}
                >
                  {row.vnd}
                </p>
                <p
                  className="text-[12px] text-right font-medium min-w-[80px]"
                  style={{ color: 'var(--green-dark)' }}
                >
                  {row.usd}
                </p>
            ...
            >
              <p
                ...
                style={{ color: 'var(--charcoal-muted)' }}
              >
                <span className="font-semibold" style={{ color: 'var(--amber)' }}>Note:</span>{' '}
        ...
        <section className={`mt-6 ${mounted ? 'animate-fade-in-up delay-400' : 'opacity-0'}`}>
          ...
          <div className="bg-white rounded-2xl shadow-soft overflow-hidden">
            ...
            {PHRASES.map((phrase, i) => (
              <div
                key={phrase.english}
                className="grid grid-cols-3 gap-2 px-4 py-3 border-t"
                style={{
                  borderColor: 'var(--cloud-dark)',
                  background: i % 2 === 1 ? 'var(--cloud)' : 'white',
                }}
          ...
          <div className="space-y-2.5">
            {EMERGENCY_NUMBERS.map((entry) => (
              <a
                key={entry.label}
                href={entry.href}
                className="flex items-center gap-3 bg-white rounded-xl p-4 shadow-soft transition-transform hover:scale-[1.01] active:scale-[0.99]"
              >
                <div
          ...
          >
            <div className="flex items-start gap-2.5">
              ...
              <p
                ...
                style={{ color: 'var(--charcoal-light)' }}
              >
                This guide is for <span className="font-semibold">informational purposes only</span> and
                does not constitute medical advice. Always consult a qualified healthcare professional
                before taking any medication. Drug regulations may change — verify current rules with
                local authorities.
              </p>
            </div>
          </div>

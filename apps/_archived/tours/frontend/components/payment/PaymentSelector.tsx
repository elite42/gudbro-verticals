...
const PAYMENT_OPTIONS: PaymentOptionConfig[] = [
  ...
  },
  {
    ...
    icon: (
      <div className="flex h-6 w-6 items-center justify-center rounded-full bg-[#A50064] text-xs font-bold text-white">
        M
      </div>
    ),
    color: 'bg-[#A50064]',
...
export function PaymentSelector({
  ...
  const availableOptions = PAYMENT_OPTIONS.filter((opt) => acceptedMethods.includes(opt.id));
    ...
    <div className={cn('space-y-3', className)}>
      ...
      <div className="space-y-2">
        {availableOptions.map((option) => {
          ...
          const fee = option.processingFee ? calculateFee(totalVnd, option.processingFee) : 0;
          ...
          return (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-foreground-muted mt-0.5 text-sm">{option.description}</p>
                {fee > 0 && (
      ...
      {/* Security note */}
      <div className="text-foreground-muted flex items-center gap-2 pt-2 text-xs">

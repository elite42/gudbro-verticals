...
must_haves:
  ...
  artifacts:
    ...
    - path: 'apps/accommodations/frontend/types/property.ts'
      ...
      exports:
        ['AccomPaymentMethod', 'PAYMENT_METHOD_CONFIG', 'BankTransferInfo']
...
1. Run `cd /Users/gianfrancodagostino/Desktop/gudbro-verticals/apps/accommodations/frontend && npx tsc --noEmit --pretty 2>&1 | head -30` to check for type errors.
2. Verify PAYMENT_METHOD_CONFIG is exported with 4 entries: `grep -c 'cash\|bank_transfer\|card\|crypto' apps/accommodations/frontend/types/property.ts` should show at least 4 matches.
3. Verify export: `grep 'export.*PAYMENT_METHOD_CONFIG' apps/accommodations/frontend/types/property.ts` should match.
   </verify>
   <done>TypeScript types include AccomPaymentMethod, PAYMENT_METHOD_CONFIG (exported with 4 entries: cash, bank_transfer, card, crypto), extended PropertyPageData with deposit/cancellation fields, BookingSubmission with paymentMethod, BookingResponse with payment details and stripeCheckoutUrl, and BookingStatus includes pending_payment/payment_failed.</done>
   </task>
